...
function Sheet({
    ...
    const access = useAccess("adcampaigntemplates");
    const viewConfig = useViewConfig("adcampaigntemplates", "sheet");
    useEffect(() => { if (entity) setSheetData(entity); }, [entity]);
    const entityId = entity?._id ?? fetchId;
    if (!viewConfig || !entityId) return null;
    const params = new URLSearchParams();
    params.set("adCampaignTemplateId", String(entityId));
    return (
        <SheetViewRenderer
            config={viewConfig}
            url="/api/realEstate/adCampaignTemplate/single"
            fetchId={fetchId}
            ...
            onDelete={onDelete}
            onRestore={onRestore}
            editPath={`/tenancy/systemSettings/adCampaignTemplates/edit?${params.toString()}`}
        />
    );
}

export default compose(
    withLanguage("src/modules/propertyManagement/clients/panel/private/adCampaignTemplates/center/sheetView/adCampaignTemplateSheetView.tsx"),
