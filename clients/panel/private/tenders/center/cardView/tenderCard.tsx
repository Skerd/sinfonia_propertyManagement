...
import Sheet from "@propertyManagementModule/clients/panel/private/tenders/center/sheetView/tenderSheetView.tsx";
...
type Props = WithLanguageType & {
    entity: Tender;
    onDelete?: (deleted?: Tender, response?: DeletedData) => void;
    onRestore?: () => void;
    hideActions?: boolean;
};
...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("tenders");
    if (hideAfterDeletion || !restore) return <></>;
    if (!read || !Object.keys(read).length) return <HiddenElement />;
    const params = new URLSearchParams();
    params.set("tenderId", entity._id);
    if (entity.name) params.set("tenderName", entity.name);
    const editPath = `/realEstate/tenders/edit?${params.toString()}`;
    return (
        <>
            <EntityCardShell onClick={() => setAction("view")}>
                <EntityTextCardHeader
                    ...
                    actionMenu={
                        <ActionMenu accessModel="tenders" deletedData={entity} onAction={(a: string) => setAction(a)} editPath={editPath} />
                    }
                />
                <Separator />
            ...
            {action === "delete" && (
                <DeleteAction accessModel="tenders" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/tender" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="tenders" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/tender/restore" />
            )}
        </>
    );
}
