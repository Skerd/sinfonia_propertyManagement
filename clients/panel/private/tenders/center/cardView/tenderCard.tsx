...
function tenderEditPath(tender: Tender) {
    const params = new URLSearchParams();
    params.set("tenderId", tender._id);
    if (tender.name) params.set("tenderName", tender.name);
    return `/realEstate/tenders/edit?${params.toString()}`;
}

type TenderCardProps = WithLanguageType & {
    entity: Tender;
    fetchId?: string;
    hideActions?: boolean;
    ...
    onRestore?: () => void;
    sheetOnly?: boolean;
    innerRef?: RefObject<WithAxiosLifecycleRef<Tender> | null>;
};
...
function TenderCard({
    ...
    return (
        <EntityCard
            ...
            entity={entity}
            fetchId={fetchId}
            singleUrl="/api/realEstate/tender/single"
            onDelete={onDelete}
            onRestore={onRestore}
            hideActions={hideActions}
            sheetOnly={sheetOnly}
            editPath={tenderEditPath}
            Sheet={Sheet}
            sheetEntityProp="entity"
            deleteUrl="/api/realEstate/tender"
            restoreUrl="/api/realEstate/tender/restore"
