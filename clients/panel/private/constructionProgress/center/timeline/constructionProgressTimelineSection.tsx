...
type ConstructionProgressTimelineSectionProps = WithLanguageType & {
    /** Route-scoped project (e.g. navigated from a project page). Overridden by quick filter. */
    projectId?: string;
    /** Route-scoped building. Overridden by quick filter. */
    edificeId?: string;
    onSelectUpdate?: (update: ConstructionProgress) => void;
};

const NODE_MIN_WIDTH_REM = 10;
const QUICK_FILTER_FIELDS = ["project", "edifice", "phase"] as const;

const UPDATE_DATE_FORMAT: Intl.DateTimeFormatOptions = {year: "numeric", month: "short", day: "numeric"};

function formatUpdateDate(value?: string): string {
    return formatDate(value, {format: UPDATE_DATE_FORMAT});
}
        ...
        <div className="w-64 overflow-hidden rounded-md">
            ...
            <div className="flex flex-col gap-y-1.5 bg-card px-3 py-2.5 text-card-foreground">
                ...
                <p className="text-xs font-medium text-primary">
                    {String(resolveLanguageKey(`phase.${update.phase}`))} · {update.progressPercent}%
                </p>
...
function ConstructionProgressTimelineSection({
    projectId,
    edificeId,
    onSelectUpdate,
    resolveLanguageKey,
}: ConstructionProgressTimelineSectionProps) {
    const [searchParams] = useSearchParams();
    const [open, setOpen] = useState(false);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(false);
    const [updates, setUpdates] = useState<ConstructionProgress[]>([]);

    const {filterProject, filterEdifice, filterPhase} = useMemo(() => {
        const fromUrl = readQuickFiltersFromUrl(searchParams, [...QUICK_FILTER_FIELDS]);
        return {
            filterProject: fromUrl.project ?? projectId ?? undefined,
            filterEdifice: fromUrl.edifice ?? edificeId ?? undefined,
            filterPhase: fromUrl.phase ?? undefined,
        };
    }, [searchParams, projectId, edificeId]);
    ...
    const fetchTimeline = useCallback(async () => {
        ...
        try {
            ...
            // Same filter DSL the list sends: project / building / phase are plain schema fields.
            const rules = [
                filterProject ? buildFilterRule("project", "equals", filterProject) : null,
                filterEdifice ? buildFilterRule("edifice", "equals", filterEdifice) : null,
                filterPhase ? buildFilterRule("phase", "equals", filterPhase) : null,
            ].filter((rule): rule is NonNullable<typeof rule> => rule !== null);
            if (rules.length) body.filter = buildFilterGroup(rules);

            const res = await apiClient.post<TableResponse<ConstructionProgress>>(
                "/api/realEstate/constructionProgress",
                body,
            );
    ...
    }, [filterProject, filterEdifice, filterPhase]);
    ...
    const dateRangeLabel = useMemo(() => {
        ...
        if (updates.length === 1) return formatUpdateDate(updates[0].updateDate);
        return `${formatUpdateDate(updates[0].updateDate)} → ${formatUpdateDate(updates[updates.length - 1].updateDate)}`;
    }, [updates]);
                                        ...
                                        {updates.map((update, index) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="mt-3 flex h-[5.25rem] w-full flex-col items-center px-2 text-center">
                                                        <p className="text-2xs font-medium tabular-nums text-muted-foreground">
                                                            {formatUpdateDate(update.updateDate)}
                                                        </p>
                                                        <p className="mt-0.5 w-full text-xs font-semibold leading-snug line-clamp-1">
                                                            {String(resolveLanguageKey(`phase.${update.phase}`))}
                                                        </p>
                                                        <p className="mt-0.5 w-full text-2xs leading-snug line-clamp-1">
                                                            {update.title}
                                                        </p>
...
export default compose(
    withLanguage("src/modules/propertyManagement/clients/panel/private/constructionProgress/center/timeline/constructionProgressTimelineSection.tsx"),
