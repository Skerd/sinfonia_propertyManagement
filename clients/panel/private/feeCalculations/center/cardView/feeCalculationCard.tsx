...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("feecalculations");
    ...
    const editPath = `/realEstate/feeCalculations/edit?${params.toString()}`;
    return (
        <>
            <EntityCardShell onClick={() => setAction("view")}>
                <EntityTextCardHeader
                    ...
                    showSubtitle={false}>{entity.status}</Badge>
                    ) : null}
                    showTitle={!!read?.name}
                    showSubtitle={false}
                    ...
                    actionMenu={
                        <ActionMenu accessModel="feecalculations" deletedData={entity} onAction={(a: string) => setAction(a)} editPath={editPath} />
                    }
                />
                <Separator />
            ...
            {action === "delete" && (
                <DeleteAction accessModel="feecalculations" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/feeCalculation" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="feecalculations" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/feeCalculation/restore" />
            )}
        </>
    );
}
