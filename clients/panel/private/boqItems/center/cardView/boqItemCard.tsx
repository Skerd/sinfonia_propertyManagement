...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("boqitems");
    if (hideAfterDeletion || !restore) return <></>;
    if (!read || !Object.keys(read).length) return <HiddenElement />;
    const params = new URLSearchParams();
    params.set("boqItemId", entity._id);
    if (entity.name) params.set("boqItemName", entity.name);
    const editPath = `/realEstate/boqItems/edit?${params.toString()}`;
    return (
        <>
            <EntityCardShell onClick={() => setAction("view")}>
                <EntityTextCardHeader
                    ...
                    actionMenu={
                        <ActionMenu accessModel="boqitems" deletedData={entity} onAction={(a: string) => setAction(a)} editPath={editPath} />
                    }
                />
                <Separator />
            ...
            {action === "delete" && (
                <DeleteAction accessModel="boqitems" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/boqItem" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="boqitems" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/boqItem/restore" />
            )}
        </>
    );
}
