...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("liquidityplans");
    ...
    const editPath = `/realEstate/liquidityPlans/edit?${params.toString()}`;
    return (
        <>
            ...
            {action === "delete" && (
                <DeleteAction accessModel="liquidityplans" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/liquidityPlan" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="liquidityplans" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/liquidityPlan/restore" />
            )}
        </>
    );
}
