...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("contractorinvoices");
    ...
    const editPath = `/realEstate/contractorInvoices/edit?${params.toString()}`;
    return (
        <>
            <EntityCardShell onClick={() => setAction("view")}>
                <EntityTextCardHeader
                    title={!!read?.name ? entity.name : null}
                    subtitle={!!read?.invoiceNumber && !!entity.invoiceNumber ? entity.invoiceNumber : undefined}
                    ...
                    actionMenu={
                        <ActionMenu accessModel="contractorinvoices" deletedData={entity} onAction={(a: string) => setAction(a)} editPath={editPath} />
                    }
                />
                <Separator />
            ...
            {action === "delete" && (
                <DeleteAction accessModel="contractorinvoices" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/contractorInvoice" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="contractorinvoices" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/contractorInvoice/restore" />
            )}
        </>
    );
}
