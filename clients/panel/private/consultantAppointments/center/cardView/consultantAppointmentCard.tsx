...
import Sheet from "@propertyManagementModule/clients/panel/private/consultantAppointments/center/sheetView/consultantAppointmentSheetView.tsx";
...
type Props = WithLanguageType & {
    ...
    onDelete?: (deleted?: ConsultantAppointment, response?: DeletedData) => void;
    onRestore?: () => void;
    hideActions?: boolean;
};
...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("consultantappointments");
    if (hideAfterDeletion || !restore) return <></>;
    if (!read || !Object.keys(read).length) return <HiddenElement />;
    const params = new URLSearchParams();
    params.set("consultantAppointmentId", entity._id);
    if (entity.name) params.set("consultantAppointmentName", entity.name);
    const editPath = `/realEstate/consultantAppointments/edit?${params.toString()}`;
    return (
        <>
            <EntityCardShell onClick={() => setAction("view")}>
                <EntityTextCardHeader
                    ...
                    actionMenu={
                        <ActionMenu accessModel="consultantappointments" deletedData={entity} onAction={(a: string) => setAction(a)} editPath={editPath} />
                    }
                />
                <Separator />
            ...
            {action === "delete" && (
                <DeleteAction accessModel="consultantappointments" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/consultantAppointment" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="consultantappointments" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/consultantAppointment/restore" />
            )}
        </>
    );
}
