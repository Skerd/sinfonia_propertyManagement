...
import Sheet from "@propertyManagementModule/clients/panel/private/costCommitments/center/sheetView/costCommitmentSheetView.tsx";
...
type Props = WithLanguageType & {
    entity: CostCommitment;
    onDelete?: (deleted?: CostCommitment, response?: DeletedData) => void;
    onRestore?: () => void;
    hideActions?: boolean;
};
...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("costcommitments");
    if (hideAfterDeletion || !restore) return <></>;
    if (!read || !Object.keys(read).length) return <HiddenElement />;
    const params = new URLSearchParams();
    params.set("costCommitmentId", entity._id);
    if (entity.name) params.set("costCommitmentName", entity.name);
    const editPath = `/realEstate/costCommitments/edit?${params.toString()}`;
    return (
        <>
            <EntityCardShell onClick={() => setAction("view")}>
                <EntityTextCardHeader
                    ...
                    actionMenu={
                        <ActionMenu accessModel="costcommitments" deletedData={entity} onAction={(a: string) => setAction(a)} editPath={editPath} />
                    }
                />
                <Separator />
            ...
            {action === "delete" && (
                <DeleteAction accessModel="costcommitments" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/costCommitment" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="costcommitments" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/costCommitment/restore" />
            )}
        </>
    );
}
