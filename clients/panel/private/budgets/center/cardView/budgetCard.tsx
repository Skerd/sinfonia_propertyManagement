...
import Sheet from "@propertyManagementModule/clients/panel/private/budgets/center/sheetView/budgetSheetView.tsx";
...
type Props = WithLanguageType & {
    entity: Budget;
    onDelete?: (deleted?: Budget, response?: DeletedData) => void;
    onRestore?: () => void;
    hideActions?: boolean;
};
...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("budgets");
    if (hideAfterDeletion || !restore) return <></>;
    if (!read || !Object.keys(read).length) return <HiddenElement />;
    const params = new URLSearchParams();
    params.set("budgetId", entity._id);
    if (entity.name) params.set("budgetName", entity.name);
    const editPath = `/realEstate/budgets/edit?${params.toString()}`;
    return (
        <>
            <EntityCardShell onClick={() => setAction("view")}>
                <EntityTextCardHeader
                    ...
                    actionMenu={
                        <ActionMenu accessModel="budgets" deletedData={entity} onAction={(a: string) => setAction(a)} editPath={editPath} />
                    }
                />
                <Separator />
            ...
            {action === "delete" && (
                <DeleteAction accessModel="budgets" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/budget" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="budgets" deleteId={entity._id} openAlert name={entity.title} confirmName={entity.title} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/budget/restore" />
            )}
        </>
    );
}
