...
function Sheet({open, onOpenChange, entity, resolveLanguageKey, hideActions = false, onDelete = () => {}, onRestore = () => {}, fetchId}: Props) {
    ...
    const access = useAccess("maintenanceworkorders");
    const viewConfig = useViewConfig("maintenanceworkorders", "sheet");
    useEffect(() => { if (entity) setSheetData(entity); }, [entity]);
    const entityId = entity?._id ?? fetchId;
    if (!viewConfig || !entityId) return null;
    const params = new URLSearchParams();
    params.set("maintenanceWorkOrderId", String(entityId));
    return (
        <SheetViewRenderer
            config={viewConfig}
            url="/api/realEstate/maintenanceWorkOrder/single"
            fetchId={fetchId}
            ...
            onDelete={onDelete}
            onRestore={onRestore}
            editPath={`/realEstate/maintenanceWorkOrders/edit?${params.toString()}`}
        />
    );
}
