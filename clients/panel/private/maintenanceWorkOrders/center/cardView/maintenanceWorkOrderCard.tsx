...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("maintenanceworkorders");
    ...
    const editPath = `/realEstate/maintenanceWorkOrders/edit?${params.toString()}`;
    return (
        <>
            <EntityCardShell onClick={() => setAction("view")}>
                <EntityTextCardHeader
                    title={!!read?.name ? entity.name : null}
                    subtitle={!!read?.type && !!entity.type ? entity.type : undefined}
                    ...
                    actionMenu={
                        <ActionMenu accessModel="maintenanceworkorders" deletedData={entity} onAction={(a: string) => setAction(a)} editPath={editPath} />
                    }
                />
                <Separator />
            ...
            {action === "delete" && (
                <DeleteAction accessModel="maintenanceworkorders" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/maintenanceWorkOrder" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="maintenanceworkorders" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/maintenanceWorkOrder/restore" />
            )}
        </>
    );
}
