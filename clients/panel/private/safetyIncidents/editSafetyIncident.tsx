...
import {editSafetyIncidentFormSchema} from "armonia/src/modules/propertyManagement/api/realEstate/private/safetyIncident/editSafetyIncident.form.validator.ts";
...
import type {EditSafetyIncidentFormType} from "armonia/src/modules/propertyManagement/api/realEstate/private/safetyIncident/safetyIncident.schema-def.ts";

export default createGenericEditPage<SafetyIncident, EditSafetyIncidentFormType>({
    languagePath: "src/modules/propertyManagement/clients/panel/private/safetyIncidents/editSafetyIncident.tsx",
    model: "safetyincidents",
    apiUrl: "/api/realEstate/safetyIncident",
    schema: editSafetyIncidentFormSchema,
    mapEntityData: (data) => ({
        ...data,
        project: (data as any).project?._id ?? (data as any).project,
        edifice: (data as any).edifice?._id ?? (data as any).edifice,
        incidentDate: (data as any).incidentDate ? new Date((data as any).incidentDate).toISOString().split("T")[0] : undefined,
