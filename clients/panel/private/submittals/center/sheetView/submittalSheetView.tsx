...
function Sheet({open, onOpenChange, entity, resolveLanguageKey, hideActions = false, onDelete = () => {}, onRestore = () => {}, fetchId}: Props) {
    ...
    const access = useAccess("submittals");
    const viewConfig = useViewConfig("submittals", "sheet");
    useEffect(() => { if (entity) setSheetData(entity); }, [entity]);
    const entityId = entity?._id ?? fetchId;
    if (!viewConfig || !entityId) return null;
    const params = new URLSearchParams();
    params.set("submittalId", String(entityId));
    return (
        <SheetViewRenderer
            config={viewConfig}
            url="/api/realEstate/submittal/single"
            fetchId={fetchId}
            ...
            onDelete={onDelete}
            onRestore={onRestore}
            editPath={`/realEstate/submittals/edit?${params.toString()}`}
        />
    );
}

export default compose(
    withLanguage("src/modules/propertyManagement/clients/panel/private/submittals/center/sheetView/submittalSheetView.tsx"),
