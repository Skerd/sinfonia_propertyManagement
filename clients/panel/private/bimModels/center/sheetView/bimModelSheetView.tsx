...
function Sheet({open, onOpenChange, entity, resolveLanguageKey, hideActions = false, onDelete = () => {}, onRestore = () => {}, fetchId}: Props) {
    ...
    const access = useAccess("bimmodels");
    const viewConfig = useViewConfig("bimmodels", "sheet");
    useEffect(() => { if (entity) setSheetData(entity); }, [entity]);
    const entityId = entity?._id ?? fetchId;
    if (!viewConfig || !entityId) return null;
    const params = new URLSearchParams();
    params.set("bimModelId", String(entityId));
    return (
        <SheetViewRenderer
            config={viewConfig}
            url="/api/realEstate/bimModel/single"
            fetchId={fetchId}
            ...
            onDelete={onDelete}
            onRestore={onRestore}
            editPath={`/realEstate/bimModels/edit?${params.toString()}`}
        />
    );
}

export default compose(
    withLanguage("src/modules/propertyManagement/clients/panel/private/bimModels/center/sheetView/bimModelSheetView.tsx"),
    withDebug(true, true),
)(Sheet);
