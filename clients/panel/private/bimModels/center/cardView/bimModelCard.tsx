...
import Sheet from "@propertyManagementModule/clients/panel/private/bimModels/center/sheetView/bimModelSheetView.tsx";
...
type Props = WithLanguageType & {
    entity: BimModel;
    onDelete?: (deleted?: BimModel, response?: DeletedData) => void;
    onRestore?: () => void;
    hideActions?: boolean;
};
...
function Card({entity: prop, onDelete: onDeleteProp, onRestore: onRestoreProp, hideActions = false}: Props) {
    ...
    const {read, restore} = useAccess("bimmodels");
    if (hideAfterDeletion || !restore) return <></>;
    if (!read || !Object.keys(read).length) return <HiddenElement />;
    const params = new URLSearchParams();
    params.set("bimModelId", entity._id);
    if (entity.name) params.set("bimModelName", entity.name);
    const editPath = `/realEstate/bimModels/edit?${params.toString()}`;
    return (
        <>
            <EntityCardShell onClick={() => setAction("view")}>
                <EntityTextCardHeader
                    title={!!read?.name ? entity.name : null}
                    subtitle={!!read?.version && !!entity.version ? entity.version : undefined}
                    badges={!!read?.importStatus && !!entity.importStatus ? (
                        <Badge variant="secondary" className={cn("text-xs", STATUS_BADGE_NEUTRAL)}>{entity.importStatus}</Badge>
                    ) : null}
                    hideActions={hideActions}
                    actionMenu={
                        <ActionMenu accessModel="bimmodels" deletedData={entity} onAction={(a: string) => setAction(a)} editPath={editPath} />
                    }
                />
                <Separator />
            ...
            {action === "delete" && (
                <DeleteAction accessModel="bimmodels" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onDelete} onCancel={() => setAction("")} url="/api/realEstate/bimModel" />
            )}
            {action === "restore" && (
                <RestoreAction accessModel="bimmodels" deleteId={entity._id} openAlert name={entity.name} confirmName={entity.name} onSuccess={onRestore} onCancel={() => setAction("")} url="/api/realEstate/bimModel/restore" />
            )}
        </>
    );
}
